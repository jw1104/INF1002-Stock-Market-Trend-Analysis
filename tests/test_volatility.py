...
class TestVolatilityAnalysis:
    """
    Unit tests for volatility analysis functions
    """
    ...
    def test_all_none_values(self):
        # test case for when all values are None
        result = analyze_volatility([None, None, None])
        assert result is None
    
    def test_mixed_none_and_valid_returns(self):
        # test case to ensure None values are properly filtered out
        returns = [None, 2.0, 3.0, -1.0, None, 1.0]
        result = analyze_volatility(returns)
        
        assert result is not None
        assert result['avg_daily_return'] == 1.25  # (2+3-1+1)/4
    
    def test_max_gain_and_loss_identification(self):
        # test case for correct identification of maximum gain and loss
    ...
    def test_negative_average_return(self):
        # test case for handling of consistently negative returns
